...
export async function getAdminDashboardData(section = "overview") {
  ...
  const [
    ...
    sectionSelect(requiredData, "liveClasses", "live classes", () => supabase.from("live_class_sessions").select("*, programs(id, title), program_levels(id, level_name), live_class_attendance(id, user_id, attendance_status)").order("scheduled_start", { ascending: false }).limit(150)),
