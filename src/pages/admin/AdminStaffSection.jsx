        ...
        {!data.staff.length ? <div className="portal-state-card"><p>No Staff accounts have been created.</p></div> : data.staff.map((person) => {
          ...
          return <article className="staff-admin-record" key={person.id}>
            ...
            <div className="staff-admin-actions">
              {person.account_status !== "active" ? <button className="button button-secondary" type="button" disabled={Boolean(busy)} onClick={() => run(`status-${person.id}`, () => setAccountStatus({ userId: person.id, status: "active", reason: "Restored by Admin after Staff account review" }), "Staff account activated.", "Staff account could not be activated.")}>Activate</button> : null}
              {person.account_status === "active" ? <button className="button button-secondary" type="button" disabled={Boolean(busy)} onClick={() => run(`status-${person.id}`, () => setAccountStatus({ userId: person.id, status: "inactive", reason: "Deactivated by Admin after Staff account review" }), "Staff account deactivated.", "Staff account could not be deactivated.")}>Deactivate</button> : null}
              {person.account_status !== "restricted" ? <button className="button button-secondary" type="button" disabled={Boolean(busy)} onClick={() => run(`status-${person.id}`, () => setAccountStatus({ userId: person.id, status: "restricted", reason: "Restricted by Admin after Staff security review" }), "Staff account restricted.", "Staff account could not be restricted.")}>Restrict</button> : null}
              {person.account_status !== "suspended" ? <button className="button button-secondary" type="button" disabled={Boolean(busy)} onClick={() => run(`status-${person.id}`, () => setAccountStatus({ userId: person.id, status: "suspended", reason: "Suspended by Admin after Staff account review" }), "Staff account suspended.", "Staff account could not be suspended.")}>Suspend</button> : null}
              {activeCase ? <span className="portal-tag"><ShieldCheck size={14} />{activeCase.case_reference}</span> : <span className="portal-tag">No active case</span>}
            </div>
