...
function SummaryCards({ data }) {
  const pending = list(data.assessments).filter((item) => !item.submission_id && (!item.due_at || new Date(item.due_at) >= new Date())).length;
  return (
    <div className="dashboard-grid academy-summary-grid">
      <article className="dashboard-card"><GraduationCap size={21} /><span>Programme</span><strong>{data.classroom?.program_title || "Pending"}</strong><small>{data.classroom?.track_title || "Track pending"}</small></article>
      <article className="dashboard-card"><School size={21} /><span>Classroom</span><strong>{data.classroom?.code || "Pending"}</strong><small>{data.classroom?.cohort_name || "Cohort pending"}</small></article>
      <article className="dashboard-card"><ClipboardCheck size={21} /><span>Work Due</span><strong>{pending}</strong><small>Published assessments not submitted</small></article>
      <article className="dashboard-card"><BarChart3 size={21} /><span>Performance</span><strong>{percent(data.performance?.overall_percentage)}</strong><small>Published grades and attendance</small></article>
    </div>
  );
}
...
function StudentAcademyDashboard({ view }) {
  ...
  const progressView = ["progress", "grades", "performance", "attendance"].includes(view);
  const filter = view === "quizzes" ? "quiz" : view === "tests" ? "test" : view === "projects" ? "project" : "all";
  return (
    <div className="portal-page academy-workspace">
      <PageHeading eyebrow="Student Portal" title={progressView ? "Progress" : view === "account" ? "Account" : "Learning"} description={`${data.classroom.program_title} | ${data.classroom.track_title} | ${data.classroom.cohort_name}`} />
      {progressView ? <SectionTabs items={[["/portal/progress", "Overview", BarChart3], ["/portal/progress/grades", "Grades", ClipboardCheck], ["/portal/progress/performance", "Performance", GraduationCap], ["/portal/progress/attendance", "Attendance", CheckCircle2]]} />
        : view === "account" ? <SectionTabs items={[["/portal/account", "Account", GraduationCap], ["/portal/account/payments", "Transactions", CreditCard]]} />
          : <SectionTabs items={[["/portal/learning", "Overview", BookOpen], ["/portal/learning/timetable", "Timetable", CalendarDays], ["/portal/learning/assignments", "Assignments", FileText], ["/portal/learning/quizzes", "Quizzes", ClipboardCheck], ["/portal/learning/tests", "Tests", Clock3]]} />}
      <SummaryCards data={data} />
      {view === "learning" ? <div className="portal-two-column"><section><h3>Modules</h3>{list(data.modules).length ? <div className="portal-list">{data.modules.map((item) => <article className="portal-record-card" key={item.id}><strong>{item.title}</strong><span>{item.description || "Published classroom module"}</span></article>)}</div> : <p>No modules have been published.</p>}</section><section><h3>What needs attention</h3><AssessmentList assessments={list(data.assessments).slice(0, 4)} /></section></div> : null}
      ...
      {progressView ? <div className="portal-two-column"><section><h3>Published grades</h3>{list(data.grades).length ? <div className="portal-list">{data.grades.map((grade) => <article className="portal-record-card" key={grade.id}><strong>{grade.title}</strong><span>{grade.score}/{grade.maximum_score} | {percent(grade.percentage)}</span><p>{grade.feedback || "No feedback added."}</p></article>)}</div> : <p>No grades have been published.</p>}</section><section><h3>Attendance</h3>{list(data.attendance).length ? <div className="portal-list">{data.attendance.map((record) => <article className="portal-record-card" key={record.id}><strong>{record.title}</strong><span>{record.session_date} | {record.status.replace(/_/g, " ")}</span></article>)}</div> : <p>No attendance has been recorded.</p>}</section></div> : null}
    </div>
  );
}
...
export function TutorAcademySection({ view = "classrooms" }) {
  const { user } = useAuth();
  const classroomsQuery = useAsyncData(getTutorClassrooms, [], { errorMessage: "Your assigned classrooms could not be loaded." });
  const [classroomId, setClassroomId] = useState(() => window.sessionStorage.getItem("zentel:tutor-classroom") || "");
  ...
  return (
    <div className="portal-page academy-workspace">
      <PageHeading eyebrow="Tutor Portal" title={view === "assessment" ? "Assessment" : view === "teaching" ? "Teaching" : "Classrooms"} description="Switch classrooms before reviewing Students or changing academic records." actions={<label className="academy-classroom-switcher"><span>Current classroom</span><select value={classroomId} onChange={(event) => setClassroomId(event.target.value)}>{classroomsQuery.data.map((item) => <option value={item.classroom_id} key={item.classroom_id}>{item.classroom_name} | {item.cohort_name}</option>)}</select></label>} />
      <SectionTabs items={[["/tutor/classrooms", "Classrooms", School], ["/tutor/teaching", "Teaching", BookOpen], ["/tutor/assessment", "Assessment", ClipboardCheck], ["/tutor/performance", "Performance", BarChart3]]} />
