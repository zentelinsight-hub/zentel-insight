...
async function provisionStaff(admin: any, input: {
  staffUserId: string;
  adminUserId: string;
  fullName: string;
  email: string;
  ...
  const { data, error } = await admin.supabase.rpc("provision_staff_account", {
    staff_user_id: input.staffUserId,
    admin_user_id: input.adminUserId,
    staff_full_name: input.fullName,
    staff_email: input.email,
    staff_phone: input.phone,
    staff_job_title: input.jobTitle,
    staff_department: input.department
  });
  if (error) throw error;
  const profile = Array.isArray(data) ? data[0] : data;
  if (!profile
    || !/^ZIF-[A-HJ-NP-Z2-9]{4}-[A-HJ-NP-Z2-9]{4}$/.test(profile.portal_id || "")
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (existingProfile?.id) {
      const { data: existingIdentity, error: identityError } = await admin.supabase.auth.admin.getUserById(existingProfile.id);
      if (identityError) throw identityError;
      const metadata = existingIdentity.user?.app_metadata || {};
      if (metadata.zentel_role !== "staff" || metadata.zentel_provisioned_by !== "admin") {
        return jsonResponse({ ok: false, error: "An account already uses this email address." }, 409, request);
      }
      const profile = await provisionStaff(admin, {
      ...
      return jsonResponse({ ok: true, staffUserId: existingProfile.id, portalId: profile.portal_id, message: "Staff account repaired as inactive." }, 200, request);
    }
    ...
    return jsonResponse({ ok: false, error: "Staff account could not be created. Please retry; incomplete Staff identities are repaired safely." }, 400, request);
  }
});
