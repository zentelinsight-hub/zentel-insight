-- Supabase Auth applies custom app metadata after the initial auth.users insert.
-- The insert trigger can therefore provision an Admin-created Staff identity as a
-- Student first. Permit only the service-role Staff transaction to correct that
-- just-created ZIS identifier to a ZIF identifier; all other Portal IDs remain
-- immutable.
...
declare
  account_role text;
  staff_reclassification boolean := false;
begin
  staff_reclassification :=
    tg_op = 'UPDATE'
    and old.portal_id ~ '^ZIS-[A-HJ-NP-Z2-9]{4}-[A-HJ-NP-Z2-9]{4}$'
    and new.portal_id ~ '^ZIF-[A-HJ-NP-Z2-9]{4}-[A-HJ-NP-Z2-9]{4}$'
    and coalesce(auth.role(), '') = 'service_role'
    and coalesce(current_setting('zentel.staff_provisioning', true), '') = 'on';

  if tg_op = 'UPDATE'
    and old.portal_id is not null
    and new.portal_id is distinct from old.portal_id
    and not staff_reclassification
  then
  ...
  if account_role is null then
    select case
      ...
      when coalesce(user_record.raw_app_meta_data ->> 'zentel_role', '') in ('staff', 'tutor')
        and coalesce(user_record.raw_app_meta_data ->> 'zentel_provisioned_by', '') = 'admin'
        then user_record.raw_app_meta_data ->> 'zentel_role'
      else 'student'
    end
    into account_role
...
begin
  ...
  into provisioned_portal_id
  from public.profiles profile
  where profile.id = $1;

  if provisioned_portal_id is null
    or provisioned_portal_id ~ '^ZIS-[A-HJ-NP-Z2-9]{4}-[A-HJ-NP-Z2-9]{4}$'
  then
  ...
  perform set_config('zentel.staff_provisioning', 'on', true);
